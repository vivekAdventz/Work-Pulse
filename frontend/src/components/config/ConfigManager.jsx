...
const SINGULAR = { Companies: 'Company', Stakeholders: 'Stakeholder', 'Team Members': 'Team Member' };
const singular = (t) => SINGULAR[t] || t.replace(/s$/, '');

export default function ConfigManager({ title, items, onAdd, onDelete, onUpdate, readOnly = false }) {
  ...
  const handleDelete = async (id, name) => {
    const ok = await confirm(`"${name}" will be permanently deleted.`, { title: `Delete ${singular(title)}` });
    if (ok) onDelete(id);
  };
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-md h-full overflow-hidden p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
          ...
          {title}
          {readOnly && (
            <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-400 normal-case">
              <span
                className="flex h-4 w-4 items-center justify-center rounded-full bg-slate-100 text-[10px] font-bold text-slate-500"
                title="Same manager reportees"
              >
                i
              </span>
              Same manager reportees
            </span>
          )}
        </h3>
      ...
      {/* Search + Add (hidden in readOnly mode) */}
      {!readOnly && (
        <div className="mb-4">
      ...
      <ul className="divide-y divide-slate-50 overflow-y-auto max-h-52 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
          ...
          <li key={item.id} className="flex items-center justify-between px-5 py-2.5 group hover:bg-slate-50/70 transition-colors">
            ...
            {!readOnly && (
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2 flex-shrink-0">
                ...
                <button onClick={() => handleDelete(item.id, item.name)} className="p-1.5 text-slate-400 hover:text-brand-red rounded-lg hover:bg-brand-red-light transition-colors">
                  <DeleteIcon />
                </button>
              </div>
            )}
        ...
        {items.length === 0 && (
          <li className="px-5 py-8 text-sm text-center text-slate-400 italic">
            {readOnly ? 'No teammates share your manager.' : 'No items yet.'}
          </li>
        )}
      </ul>
