...
export default function TaskKeepView({ user, fullDb, setFullDb, isManager, showToast, onOpenTimeEntryForm }) {
  ...
  const handleUpdateTask = async (dayId, taskId, field, value) => {
    ...
    try {
      ...
      // Show toast when task is assigned
                    ...
                    <div key={task.id} className="group/task flex items-start gap-4 relative">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) => handleUpdateTask(day.id, task.id, 'title', e.target.value)}
                          onBlur={(e) => { if (canEditTask && e.target.value !== task.title) handleUpdateTask(day.id, task.id, 'title', e.target.value); }}
                          placeholder="Task details..."
                          className={`w-full bg-transparent border-none p-0 text-sm focus:ring-0 focus:outline-none font-semibold ${task.status === 'done' ? 'line-through text-slate-400 decoration-2' : 'text-slate-700'}`}
                        />
